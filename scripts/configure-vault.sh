...
kubectl exec -n vault vault-0 -- sh -c 'vault policy write ibm-verify-policy - <<EOF
...
kubectl exec -n vault vault-0 -- vault write auth/kubernetes/role/ibm-verify \
...
kubectl exec -n vault vault-0 -- vault read auth/kubernetes/role/ibm-verify
echo ""
